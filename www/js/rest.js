var REST = {
  ...
  KAMUS: {
    createFolder: function (local, callback = function () { }) {
      webix.ajax().post(REST.url + "/kamus?local=" + local, {}, {
        error: function (t, d, x) {
          callback({ error: { type: "server", msg: d.json() } })
        },
        success: function (t, d, x) {
          // console.log(REST.url + "/kamus?local=" + local)
          callback(d.json())
        }
      })
    }
  ...
  TinyDB: {
    listItems: function (table, local = false, callback = function () { }) {
